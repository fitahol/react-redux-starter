...
export function expireFormat(num) {
	let expireSec = num
	if (expireSec - 0) {
		expireSec = Math.floor(expireSec)
		expireSec = `${toDou(Math.floor(expireSec / 60))}:${toDou(expireSec % 60)}`
	}
	return expireSec
}
...
export function logEvent(e, note) {
	const hostName = location.hostname
	let domain = `http://${hostName}`
	if (location.hostname === '91atm.local.com') {
		domain = 'http://91atm.aa123bb.com'
	}
	const atmInfo = localStorage.getItem('91atm') || '{}'
	let userParam = ''
	const user = localStorage.getItem('user') || ''
	const appVersion = JSON.parse(atmInfo).app_version || ''
	const deviceId = JSON.parse(atmInfo).device_id || ''
	if (user) {
		userParam = `&user=${user}`
	} else if (note) {
		userParam = '&user=noUserInfo'
	}
	...
	let deviceIdParams = ''
	if (deviceId) {
		deviceIdParams = `&deviceId=${deviceId}`
	}
	fetch(`${domain}/firmware/client/log/?event=${e}${userParam}${appVersionParam}${deviceIdParams}`)
}
